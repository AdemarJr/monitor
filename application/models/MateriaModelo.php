...
;            $this->db->from('MATERIA');
            ...
            if (!empty($this->session->userData('idClienteSessao')))
                $this->db->where('MATERIA.SEQ_CLIENTE', $this->session->userData('idClienteSessao'));
            $this->db->where('SEQ_VEICULO > 0');
            $this->db->where("DATA_PUB_NUMERO BETWEEN ".$semana_anterior." AND ".date('Ymd'));
            $this->db->from('MATERIA');
            $this->db->select('MATERIA.SEQ_MATERIA');
            $totalCadastrada = count($this->db->get()->result_array());
            ...
            $this->db->select('MATERIA.SEQ_MATERIA');
            $this->db->from('MATERIA');
            $this->db->where("DATA_PUB_NUMERO", date('Ymd'));
...
//                    "%' or upper(MATERIA.TEXTO_MATERIA) like '%".strtoupper($texto)."%')");
    ...
    public function listaMateriaRelatorioExp($datai,$dataf,$veiculo,$portal,$origens,$avaliacoes,$texto,$setor,$radio,$tipo,$tv,$tipoMat,$horair=NULL,
                                          $horafr=NULL,$datai2=NULL,$dataf2=NULL,$release=NULL,$grupo=NULL,$usuario=NULL,$isrelease=NULL) {
        $periodoIni = preg_replace('#(\d{2})/(\d{2})/(\d{4})#', '$3$2$1', $datai);
        $periodoFim = preg_replace('#(\d{2})/(\d{2})/(\d{4})#', '$3$2$1', $dataf);
        if(!empty($horair) and !empty($horafr) and !empty($datai2) and !empty($dataf2)) {
        ...
        if(!empty($tipoMat)){
            $this->db->where_in('MATERIA.TIPO_MATERIA', $tipoMat);
        }
        if(!empty($release)){
        ...
        $this->db->order_by('MATERIA.TIPO_MATERIA ASC,DATA_PUB_NUMERO ASC,1 ASC,MATERIA.SEQ_MATERIA ASC, MATERIA.IND_AVALIACAO ASC');
        $this->db->select("concat(DATA_PUB_NUMERO,replace(HORA_MATERIA,':','')) as DATA_NUMERO, MATERIA.SEQ_RELEASE,SETOR.DESC_SETOR,SETOR.SIG_SETOR,OLD_PASSWORD(MATERIA.SEQ_MATERIA) AS CHAVE,DATE_FORMAT(DATA_MATERIA_PUB, '%d/%m/%Y') as DIA, MATERIA.*");
...
//            die();
        ...
        if(!empty($release) and $release > 0){
...
//                    "%' or upper(MATERIA.TEXTO_MATERIA) like '%".strtoupper($texto)."%')");
        ...
        if(!empty($setor)){
            $this->db->where_in('MATERIA.SEQ_SETOR', $setor);
        }else{
            $this->db->where("(MATERIA.SEQ_SETOR<>83 or IND_FILTRO='N')");
        }
        if(!empty($veiculo)){
...
//        $this->db->where("DATE_FORMAT(DATA_MATERIA_PUB, '%c')=".$mes);
    ...
    public function getConsulta($chave=null,$limit= null, $offset = null)
    {
        ...
        $dtInicio = explode(' ', $dataNota->DT_INICIO);
        $dtInicio = str_replace('-', '', $dtInicio);
        $dtFim = explode(' ', $dataNota->DT_FIM);
    ...
    //     )
            ...
            if (isset($_SESSION['filtro'])) {
                if ($_SESSION['filtro']['ordem'] == '1') {
                  $this->db->order_by('IND_AVALIACAO desc,1 DESC, SEQ_MATERIA DESC');      
                } else {
                  $this->db->order_by('IND_AVALIACAO desc,DATA_PUB_NUMERO desc,1 DESC, SEQ_MATERIA DESC');      
                }
            } else {
                $this->db->order_by('IND_AVALIACAO desc,DATA_PUB_NUMERO desc,1 DESC, SEQ_MATERIA DESC');    
            }
...
//            $this->db->where();
        ...
        $this->db->where("DATA_PUB_NUMERO BETWEEN ".$dtInicio[0]." and ".$dtFim[0]);
...
//            $this->db->where('MATERIA.SEQ_USUARIO', $this->session->userData('idUsuario'));
        ...
        if (isset($_SESSION['FILTROS']['tipoClipador'])) {
            $this->db->where_in('MATERIA.SEQ_USUARIO', $_SESSION['FILTROS']['tipoClipador']);
        }
        if (isset($_SESSION['FILTROS']['tipoMat'])) {
            $this->db->where_in('MATERIA.TIPO_MATERIA', $_SESSION['FILTROS']['tipoMat']);
        }
        $this->db->where('MATERIA.DATA_PUB_NUMERO BETWEEN '.$semana_anterior.' AND '.$hoje.' ');
...
//            $this->db->or_like("RADIO.NOME_RADIO", $_POST["search"]["value"]);
            $this->db->like("MATERIA.TIT_MATERIA", $_SESSION['FILTROS']['texto'], 'both');
            $this->db->or_like("MATERIA.PC_MATERIA", $_SESSION['FILTROS']['texto']);
...
//            $this->db->or_like("MATERIA.PROGRAMA_MATERIA", $_POST["search"]["value"]);
            //$this->db->or_like("MATERIA.TEXTO_MATERIA", $_SESSION['FILTROS']['texto']);
