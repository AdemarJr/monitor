    ...
    function getDescSituacaoAvulso($id)
{
    ...
    {
        $this->db->where("date_format(MATERIA.DATA_MATERIA_PUB,'%Y%m')",$mes);
        $this->db->where('MATERIA.SEQ_CLIENTE', $this->session->userData('idClienteSessao'));
        $this->db->where('MATERIA.IND_AVALIACAO',$avaliacao);
...
//        $this->db->from('MATERIA');
    ...
    public function getMaiorDataAlerta($chave=null)
    {
        ...
        $this->db->where("DATE_FORMAT(DATA_MATERIA_PUB, '%Y%m%d') BETWEEN DATE_FORMAT(NOTA.DT_INICIO, '%Y%m%d') and DATE_FORMAT(NOTA.DT_FIM, '%Y%m%d')");
...
//        $periodoFim = preg_replace('#(\d{2})/(\d{2})/(\d{4}) - (\d{2}):(\d{2})#', '$3$2$1$4$5', $dataf);
        ...
        $slq = "SELECT A.SEQ_USUARIO,A.NOME_USUARIO, SUM(IF(A.TIPO_MATERIA='I',QTD,0)) AS IMPRESSO,SUM(IF(A.TIPO_MATERIA='S',QTD,0)) AS INTERNET, "
            ...
            ."AND DATE_FORMAT(DATA_MATERIA_CAD, '%Y%m%d%H%i') BETWEEN ".trim($periodoIni)." AND ".trim($periodoFim).(!empty($idCliente)? " AND A.SEQ_CLIENTE in ( ".$idCliente.")":'')." GROUP BY B.SEQ_USUARIO,B.NOME_USUARIO,A.TIPO_MATERIA) A GROUP BY A.SEQ_USUARIO,NOME_USUARIO";
...
//        $periodoFim .= preg_replace("#(\d{2}):(\d{2})#", "$1$2", $horafr);
    ...
    ." WHERE A.IND_AVALIACAO<>'T' AND A.SEQ_SETOR NOT IN (316,317,318) AND "
    .($setor>0?' A.SEQ_SETOR='.$setor. ' AND':"")
    ...
    ." DATE_FORMAT(DATA_MATERIA_PUB, '%Y%m%d') BETWEEN ".trim($periodoIni)." AND ".trim($periodoFim).(!empty($idCliente)? " AND A.SEQ_CLIENTE =".$idCliente:'')
...
//        $periodoFim = preg_replace('#(\d{2})/(\d{2})/(\d{4}) - (\d{2}):(\d{2})#', '$3$2$1$4$5', $dataf);
        ...
        $slq = "SELECT MIN(DATA_MATERIA_CAD) AS MENOR_MAT,MAX(DATA_MATERIA_CAD) AS MAIOR_MAT"
            ...
            ."LEFT JOIN PORTAL D ON A.SEQ_PORTAL=D.SEQ_PORTAL WHERE B.LOGIN_USUARIO<>'admin' AND"
            ."(A.SEQ_VEICULO IS NOT NULL OR A.SEQ_PORTAL IS NOT NULL OR A.SEQ_RADIO IS NOT NULL OR A.SEQ_TV IS NOT NULL) AND A.SEQ_SETOR IS NOT NULL AND A.IND_AVALIACAO IS NOT NULL "
            ."AND DATE_FORMAT(DATA_MATERIA_CAD, '%Y%m%d%H%i') BETWEEN ".trim($periodoIni)." AND ".trim($periodoFim).(!empty($idCliente)? " AND A.SEQ_CLIENTE in ( ".$idCliente.")":'')." AND A.SEQ_USUARIO=".$idUsuario;
...
//        if (!empty($idCliente)){ echo $slq;die;}
    ...
    public function quantitativoSetor($datai=NULL,$dataf=NULL,$idCliente=NULL,$tipoMateria=NULL,$grupo=NULL,
                                      $excluirSetor=NULL,$setores=NULL)
    {
        $periodoIni = preg_replace('#(\d{2})/(\d{2})/(\d{4})#', '$3$2$1', $datai);
        $periodoFim = preg_replace('#(\d{2})/(\d{2})/(\d{4})#', '$3$2$1', $dataf);
        $whereGrupo = '';
...
//              "(SELECT C.NOME_PORTAL AS DESCRICAO,A.IND_AVALIACAO, COUNT(*) as QTD FROM MATERIA A "
            ...
            . "AND A.SEQ_SETOR IS NOT NULL AND A.IND_AVALIACAO IS NOT NULL ) AND DATE_FORMAT(DATA_MATERIA_PUB, '%Y%m%d') BETWEEN " .
            trim($periodoIni) . " AND " . trim($periodoFim) . " AND A.SEQ_CLIENTE =" . $idCliente
            . " GROUP BY A.TIPO_MATERIA,B.DESC_SETOR,A.IND_AVALIACAO ) X GROUP BY TIPO_MATERIA,DESC_SETOR ORDER BY 1 ASC,6 DESC ";
//        echo $slq;
//        die;
        $query = $this->db->query($slq);
        return $query->result_array();
                                      ...
                                      $excluirSetor=NULL,$setores=NULL)
    {
        ...
        $slq = "SELECT X.TIPO_MATERIA,X.DESC_SETOR, SUM(IF(X.IND_AVALIACAO='P',X.QTD,0)) AS POSITIVO, SUM(IF(X.IND_AVALIACAO='N',X.QTD,0)) AS NEGATIVO, "
            ...
            . "AND A.SEQ_SETOR IS NOT NULL AND A.IND_AVALIACAO IS NOT NULL ) AND DATE_FORMAT(DATA_MATERIA_PUB, '%Y%m%d') BETWEEN " . trim($periodoIni) . " AND " . trim($periodoFim) . " AND A.SEQ_CLIENTE =" . $idCliente
            . " GROUP BY A.TIPO_MATERIA,B.DESC_SETOR,A.IND_AVALIACAO ) X GROUP BY TIPO_MATERIA,DESC_SETOR ORDER BY 1 ASC,6 DESC ";
    ...
    public function quantitativoSetor2Cliente($datai=NULL,$dataf=NULL)
    {
        ...
        $slq = "SELECT X.NOME_CLIENTE, SUM(IF(X.IND_AVALIACAO='P',X.QTD,0)) AS POSITIVO, SUM(IF(X.IND_AVALIACAO='N',X.QTD,0)) AS NEGATIVO, "
            ...
            . "LEFT JOIN PORTAL D ON A.SEQ_PORTAL=D.SEQ_PORTAL  WHERE B.IND_ESTR='S'  AND ((A.SEQ_VEICULO IS NOT NULL OR A.SEQ_PORTAL IS NOT NULL OR A.SEQ_RADIO IS NOT NULL OR A.SEQ_TV IS NOT NULL) "
            . "AND A.SEQ_SETOR IS NOT NULL AND A.IND_AVALIACAO IS NOT NULL ) AND DATE_FORMAT(DATA_MATERIA_PUB, '%Y%m%d') BETWEEN " . trim($periodoIni) . " AND " . trim($periodoFim)
        ...
        $excluirSetor=NULL,$setores=NULL)
    {
        ...
        $slq = "SELECT X.DESCRICAO, SUM(IF(X.IND_AVALIACAO='P',X.QTD,0)) AS POSITIVO, SUM(IF(X.IND_AVALIACAO='N',X.QTD,0)) AS NEGATIVO, "
            . "SUM(IF(X.IND_AVALIACAO='T',X.QTD,0)) AS NEUTRO, SUM(X.QTD) AS TOTAL FROM "
            . "(SELECT C.FANTASIA_VEICULO AS DESCRICAO,A.IND_AVALIACAO,COUNT(*) as QTD FROM MATERIA A "
            . "INNER JOIN VEICULO C ON A.SEQ_VEICULO=C.SEQ_VEICULO "
            . "WHERE ".$setor." FIND_IN_SET(83, A.SEQ_SETOR)=0 AND A.TIPO_MATERIA='" . $tipoMateria . "' AND ((A.SEQ_VEICULO IS NOT NULL OR A.SEQ_PORTAL IS NOT NULL OR A.SEQ_RADIO IS NOT NULL OR A.SEQ_TV IS NOT NULL) "
            . "AND A.SEQ_SETOR IS NOT NULL AND A.IND_AVALIACAO IS NOT NULL ) ".$release." AND DATE_FORMAT(DATA_MATERIA_PUB, '%Y%m%d') BETWEEN " . trim($periodoIni) . " AND " . trim($periodoFim) . " AND A.SEQ_CLIENTE =" . $idCliente
            ...
            $slq = "SELECT X.DESCRICAO, SUM(IF(X.IND_AVALIACAO='P',X.QTD,0)) AS POSITIVO, SUM(IF(X.IND_AVALIACAO='N',X.QTD,0)) AS NEGATIVO, "
                . "SUM(IF(X.IND_AVALIACAO='T',X.QTD,0)) AS NEUTRO, SUM(X.QTD) AS TOTAL FROM "
                . "(SELECT C.NOME_RADIO AS DESCRICAO,A.IND_AVALIACAO,COUNT(*) as QTD FROM MATERIA A "
            ...
            $slq = "SELECT X.DESCRICAO, SUM(IF(X.IND_AVALIACAO='P',X.QTD,0)) AS POSITIVO, SUM(IF(X.IND_AVALIACAO='N',X.QTD,0)) AS NEGATIVO, "
                . "SUM(IF(X.IND_AVALIACAO='T',X.QTD,0)) AS NEUTRO, SUM(X.QTD) AS TOTAL FROM "
                . "(SELECT C.NOME_TV AS DESCRICAO,A.IND_AVALIACAO,COUNT(*) as QTD FROM MATERIA A "
    ...
    )
    {
        ...
        if ($tipoMateria == 'I') {
            $slq = "SELECT X.BAIRRO_CRIME, SUM(IF(X.IND_AVALIACAO='P',X.QTD,0)) AS POSITIVO, SUM(IF(X.IND_AVALIACAO='N',X.QTD,0)) AS NEGATIVO, "
                ...
                . "AND A.SEQ_SETOR IS NOT NULL AND A.IND_AVALIACAO IS NOT NULL ) ".$release." AND DATE_FORMAT(DATA_MATERIA_PUB, '%Y%m%d') BETWEEN " . trim($periodoIni) . " AND " . trim($periodoFim) . " AND A.SEQ_CLIENTE =" . $idCliente . $strSeguranca
...
//        die;
    ...
    public function quantitativoRelease($datai=NULL,$dataf=NULL,$idCliente=NULL,$grupo=NULL,$datae=NULL,$dataef=NULL, $tipo=NULL)
    {
        $periodoIni = preg_replace('#(\d{2})/(\d{2})/(\d{4})#', '$3$2$1', $datai);
        $periodoFim = preg_replace('#(\d{2})/(\d{2})/(\d{4})#', '$3$2$1', $dataf);
...
*/
        ...
        ." OR B.SEQ_TV IS NOT NULL) AND DATE_FORMAT(DATA_MATERIA_PUB, '%Y%m%d') BETWEEN " . trim($periodoIni) . " AND " . trim($periodoFim).")"
        ." AS quantidade "
        ." FROM RELEASE_MATERIA A "
        ." WHERE A.SEQ_CLIENTE =" . $idCliente .$periodoEnvio
        ." ORDER BY 3 DESC ";
...
//        die;
        ...
        $setores=NULL)
    {
        ...
        ." WHERE A.SEQ_CLIENTE =1 and ".$setor." DATA_PUB_NUMERO between ".$periodoIni." and ".$periodoFim.$tipo
        ." ORDER BY 3,4 ";
...
//        die;
    ...
    public function quantitativoReleaseSetor($datai=NULL,$dataf=NULL,$idCliente=NULL,$grupo=NULL,$datae=NULL,$dataef=NULL)
    {
        $periodoIni = preg_replace('#(\d{2})/(\d{2})/(\d{4})#', '$3$2$1', $datai);
        $periodoFim = preg_replace('#(\d{2})/(\d{2})/(\d{4})#', '$3$2$1', $dataf);
    ...
    public function quantitativoReleasePauta($datai=NULL,$dataf=NULL,$idCliente=NULL,$grupo=NULL,$datae=NULL,$dataef=NULL)
    {
        $periodoIni = preg_replace('#(\d{2})/(\d{2})/(\d{4})#', '$3$2$1', $datai);
        $periodoFim = preg_replace('#(\d{2})/(\d{2})/(\d{4})#', '$3$2$1', $dataf);
...
//        die;
    ...
    public function quantitativoVeiculoRadioTempo($datai=NULL,$dataf=NULL,$idCliente=NULL,$excluirSetor=NULL,$setores=NULL)
    {
        $periodoIni = preg_replace('#(\d{2})/(\d{2})/(\d{4})#', '$3$2$1', $datai);
        $periodoFim = preg_replace('#(\d{2})/(\d{2})/(\d{4})#', '$3$2$1', $dataf);
        ...
        if (!empty($excluirSetor) and $excluirSetor=='S' and !empty($setores)){
            ...
            $slq = "SELECT X.DESCRICAO, SUM(IF(X.IND_AVALIACAO='P',X.QTD,0)) AS POSITIVO, SUM(IF(X.IND_AVALIACAO='N',X.QTD,0)) AS NEGATIVO, "
                ...
                . "WHERE ".$setor." FIND_IN_SET(83, A.SEQ_SETOR)=0 AND A.TIPO_MATERIA='R' AND ((A.SEQ_VEICULO IS NOT NULL OR A.SEQ_PORTAL IS NOT NULL OR A.SEQ_RADIO IS NOT NULL OR A.SEQ_TV IS NOT NULL) "
                . "AND A.SEQ_SETOR IS NOT NULL AND A.IND_AVALIACAO IS NOT NULL ) AND DATE_FORMAT(DATA_MATERIA_PUB, '%Y%m%d') BETWEEN " . trim($periodoIni) . " AND " . trim($periodoFim) . " AND A.SEQ_CLIENTE =" . $idCliente
                . " GROUP BY C.NOME_RADIO,A.IND_AVALIACAO ) X GROUP BY DESCRICAO ORDER BY 5 DESC ";
                                         ...
                                         $isrelease=NULL)
    {
        ...
        $sql =" SELECT TRIM(X.DESCRICAO) AS radio, X.PROGRAMA_MATERIA as programa, X.VALOR as vlunitario ,"
            ...
            ." AND DATE_FORMAT(DATA_MATERIA_PUB, '%Y%m%d') BETWEEN " . trim($periodoIni) . " AND " . trim($periodoFim) . " AND A.SEQ_CLIENTE =". $idCliente
    ...
    public function quantitativoVeiculoTvTempo($datai=NULL,$dataf=NULL,$idCliente=NULL,$excluirSetor=NULL,$setores=NULL)
    {
        $periodoIni = preg_replace('#(\d{2})/(\d{2})/(\d{4})#', '$3$2$1', $datai);
        $periodoFim = preg_replace('#(\d{2})/(\d{2})/(\d{4})#', '$3$2$1', $dataf);
